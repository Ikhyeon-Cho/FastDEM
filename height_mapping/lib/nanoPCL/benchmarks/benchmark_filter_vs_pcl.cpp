  ...
  for (size_t NUM_POINTS : point_counts) {
    ...
    // nanoPCL voxelGrid (copy)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        for (size_t i = 0; i < NUM_POINTS; ++i) {
          cloud.push_back(nanopcl::Point(pos(gen), pos(gen), pos(gen)),
  ...
  for (size_t NUM_POINTS : point_counts) {
    ...
    // nanoPCL cropZ (copy)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered = nanopcl::filters::cropZ(cloud, Z_MIN, Z_MAX);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
    ...
    // nanoPCL cropZ (move)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered =
            nanopcl::filters::cropZ(std::move(cloud), Z_MIN, Z_MAX);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
