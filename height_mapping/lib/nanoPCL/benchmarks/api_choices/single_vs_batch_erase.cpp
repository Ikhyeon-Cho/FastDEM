...
std::vector<size_t> randomIndices(size_t cloud_size, size_t num_indices,
                                  unsigned seed) {
  std::mt19937 gen(seed);
  std::uniform_int_distribution<size_t> dist(0, cloud_size - 1);
  std::set<size_t> unique;
  while (unique.size() < num_indices) unique.insert(dist(gen));
...
PointCloud createTestCloud(size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    cloud.add(Point(fi, fi * 2, fi * 3), Intensity(fi * 0.1f), Ring(i % 64),
              Time(fi * 0.001f));
  }
  return cloud;
}
...
double benchSingleErase(size_t cloud_size, const std::vector<size_t>& indices) {
  ...
  for (size_t idx : sorted) cloud.erase(idx);
  auto end = Clock::now();
