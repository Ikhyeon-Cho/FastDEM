...
inline Eigen::Vector3f computeNormal(const Point& query_pt,
                                      ...
                                      float radius, size_t min_neighbors) {
  if (!query_pt.allFinite()) return Eigen::Vector3f::Zero();
  ...
  if (acc.count < min_neighbors) return Eigen::Vector3f::Zero();
  ...
  if (scale <= std::numeric_limits<float>::min()) return Eigen::Vector3f::Zero();
  ...
  if (normal.dot(-query_pt) < 0) normal = -normal;
  return normal;
}
}  // namespace bench_detail
...
void estimateNormals_OutParam(const PointCloud& cloud, float radius,
