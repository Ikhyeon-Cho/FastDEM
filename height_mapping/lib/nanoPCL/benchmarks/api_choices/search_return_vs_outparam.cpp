...
void printHeader(const std::string& scenario, size_t cloud_size,
                 size_t num_queries, float radius) {
  std::cout << "\n" << std::string(75, '=') << "\n";
  std::cout << scenario << "\n";
...
void printResults(const std::vector<std::string>& names,
  ...
  for (size_t i = 0; i < names.size(); ++i) {
    ...
    std::cout << std::left << std::setw(35) << names[i] << std::right
              ...
              << std::setw(12) << "~" << (total_results[i] > 0 ?
                 std::to_string(total_results[i] / 30) : "0")  // avg over rounds
              << "  " << ratio_str << "\n";
  }
}
...
// Main Benchmark
// =============================================================================
void runRealisticBenchmark(size_t cloud_size, float cloud_range,
                           float grid_resolution, float search_radius,
                           int rounds) {
  // Generate data
