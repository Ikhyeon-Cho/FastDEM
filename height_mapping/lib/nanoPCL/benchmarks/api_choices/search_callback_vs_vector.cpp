...
float searchVector(const PointCloud& cloud, const search::VoxelHash& searcher,
                   const std::vector<Point>& queries, float radius,
...
float searchCallback(const PointCloud& cloud, const search::VoxelHash& searcher,
                     const std::vector<Point>& queries, float radius) {
  ...
  for (const auto& query : queries) {
    ...
    searcher.radius(query, radius,
                    [&acc](uint32_t /*idx*/, const Point& pt, float /*dist_sq*/) {
                      acc.add(pt);
                    });
...
float searchCallbackRef(const PointCloud& cloud, const search::VoxelHash& searcher,
                        const std::vector<Point>& queries, float radius) {
  (void)cloud;
...
// Benchmark Runner
// =============================================================================
void printHeader(const std::string& scenario, size_t cloud_size,
                 size_t num_queries, float radius, const std::string& cache_state) {
  std::cout << "\n" << std::string(80, '=') << "\n";
...
void runBenchmark(size_t cloud_size, float cloud_range, size_t num_queries,
                  float search_radius, int rounds, bool cold_cache) {
  // Generate data
  ...
  printHeader("Normal Estimation Workload", cloud_size, num_queries,
              search_radius, cold_cache ? "COLD CACHE" : "WARM CACHE");
