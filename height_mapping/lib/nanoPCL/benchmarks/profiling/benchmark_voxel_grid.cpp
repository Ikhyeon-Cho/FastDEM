...
PointCloud generateCloud(size_t num_points, int num_channels, uint32_t seed = 42) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    cloud.add(Point(pos_dist(rng), pos_dist(rng), pos_dist(rng)));
    if (num_channels >= 1) cloud.intensity().back() = val_dist(rng);
    if (num_channels >= 2) cloud.time().back() = val_dist(rng);
    if (num_channels >= 3) cloud.ring().back() = ring_dist(rng);
  }
