...
//   1. P2P ICP (with OpenMP parallelized correspondence search)
//   2. P2Plane ICP (with OpenMP parallelized correspondence search)
//
// Both algorithms now use the same parallel pattern for fair comparison.
...
npcl::PointCloud generatePlaneCloud(size_t num_points, std::mt19937& gen) {
  // Generate points on a slightly noisy plane (z = 0.1*x + 0.05*y + noise)
...
npcl::PointCloud generate3DCloud(size_t num_points, std::mt19937& gen) {
  // Generate 3D structured cloud (sphere + noise) - fair for both P2P and P2Plane
...
void run_benchmark(size_t num_points, CloudType cloud_type = CloudType::SPHERICAL) {
  ...
  // Generate target based on cloud type
  ...
  npcl::PointCloud source;
  source.reserve(num_points);
  for (size_t i = 0; i < num_points; ++i) {
    source.add((true_T * target[i].cast<double>()).cast<float>());
  }

  // Estimate normals for P2Plane (on target)
  auto target_with_normals = target;  // Copy for normal estimation
  std::cout << "\n  Estimating normals... " << std::flush;
  auto norm_start = Clock::now();
  npcl::geometry::estimateNormals(target_with_normals, 0.5f);
  auto norm_end = Clock::now();
  double norm_ms =
      std::chrono::duration<double, std::milli>(norm_end - norm_start).count();
  std::cout << "done (" << std::fixed << std::setprecision(2) << norm_ms
            << " ms)\n";

  // Build search index
  npcl::search::KdTree tree;
  std::cout << "  Building KdTree... " << std::flush;
  auto tree_start = Clock::now();
  tree.build(target_with_normals);
  auto tree_end = Clock::now();
  double tree_ms =
      std::chrono::duration<double, std::milli>(tree_end - tree_start).count();
  std::cout << "done (" << tree_ms << " ms)\n";
  ...
  auto p2p_result = measure(
      [&]() {
        return npcl::registration::icp(
            source, target, tree, Eigen::Isometry3d::Identity(), cfg);
      },
      5);
  std::cout << "done\n";
  ...
  auto p2plane_result = measure(
      [&]() {
        return npcl::registration::icpPlane(
            source, target_with_normals, tree, Eigen::Isometry3d::Identity(),
  ...
  std::cout << "  │ Per-iter (ms)   │ " << std::setw(14)
            << p2p_result.per_iter_ms << " │ " << std::setw(14)
            << p2plane_result.per_iter_ms << " │\n";
  ...
  // Total speedup considering fewer iterations
...
int main(int argc, char** argv) {
  std::cout
      << "╔═══════════════════════════════════════════════════════════════════════════╗\n";
  std::cout
      << "║         ICP Benchmark: Point-to-Point vs Point-to-Plane                   ║\n";
  std::cout
      << "╚═══════════════════════════════════════════════════════════════════════════╝\n";

#ifdef _OPENMP
  std::cout << "\n  [Compiled WITH OpenMP - Parallel correspondence search "
               "enabled]\n";
#else
  std::cout << "\n  [Compiled WITHOUT OpenMP - Sequential mode]\n";
#endif
  ...
  // Test with 3D spherical structure (fair comparison)
  std::cout << "\n\n══════════════════════════════════════════════════════════════════════════\n";
  std::cout << "  TEST 1: 3D SPHERICAL STRUCTURE (Fair comparison for both algorithms)\n";
  std::cout << "══════════════════════════════════════════════════════════════════════════\n";
  for (size_t n : sizes) {
    run_benchmark(n, CloudType::SPHERICAL);
  }

  // Test with planar structure (P2Plane advantage)
  std::cout << "\n\n══════════════════════════════════════════════════════════════════════════\n";
  std::cout << "  TEST 2: PLANAR STRUCTURE (P2Plane has theoretical advantage)\n";
  std::cout << "══════════════════════════════════════════════════════════════════════════\n";
  for (size_t n : sizes) {
