...
void printResult(const std::string& name, double time_ms, size_t in_pts,
                 size_t out_pts, double baseline_ms = 0) {
...
int main() {
  ...
  for (size_t NUM_POINTS : point_counts) {
    ...
    // nanoPCL voxelGrid (copy)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        npcl::PointCloud cloud("benchmark");
        cloud.reserve(NUM_POINTS);
        cloud.enableIntensity();
        for (size_t i = 0; i < NUM_POINTS; ++i) {
          cloud.add(npcl::Point(pos(gen), pos(gen), pos(gen)),
                          npcl::Intensity(val(gen)));
        }

        Timer t;
        auto filtered = npcl::filters::voxelGrid(cloud, VOXEL_SIZE);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / ITERATIONS;
      printResult("[nanoPCL] voxelGrid (copy)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
    ...
    // nanoPCL voxelGrid (move)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered =
            npcl::filters::voxelGrid(std::move(cloud), VOXEL_SIZE);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / ITERATIONS;
      printResult("[nanoPCL] voxelGrid (move)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
  }
  ...
  for (size_t NUM_POINTS : point_counts) {
    ...
    npcl::Point min_p(-10, -10, -5);
    npcl::Point max_p(10, 10, 5);
    ...
    // nanoPCL cropBox (copy)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered = npcl::filters::cropBox(cloud, min_p, max_p);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / ITERATIONS;
      printResult("[nanoPCL] cropBox (copy)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
    ...
    // nanoPCL cropBox (move)
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered =
            npcl::filters::cropBox(std::move(cloud), min_p, max_p);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / ITERATIONS;
      printResult("[nanoPCL] cropBox (move)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
  }
  ...
  for (size_t NUM_POINTS : ror_point_counts) {
    ...
    // nanoPCL radiusOutlierRemoval
    {
      ...
      for (int iter = 0; iter < 5; ++iter) {
        ...
        Timer t;
        auto filtered =
            npcl::filters::radiusOutlierRemoval(cloud, RADIUS, MIN_NEIGHBORS);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / 5;
      printResult("[nanoPCL] radiusOutlierRemoval", nano_time, NUM_POINTS,
                  nano_out, pcl_time);
    }
  }
