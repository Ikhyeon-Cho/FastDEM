...
void run_benchmark(size_t num_points) {
  ...
  npcl::PointCloud nano_target, nano_source;
  for (size_t i = 0; i < num_points; ++i) {
    npcl::Point pt(dist(gen), dist(gen), dist(gen));
    nano_target.add(pt);
    nano_source.add((true_T * pt.cast<double>()).cast<float>());
  }
  ...
  // nanoPCL ICP
  // --------------------------------------------------------------------------
  npcl::search::KdTree nano_tree;
  double nano_build = measure_ms([&]() { nano_tree.build(nano_target); }, 3);

  npcl::registration::ICPConfig cfg;
  cfg.max_iterations = 50;
  cfg.max_correspondence_dist = 1.0f;

  npcl::registration::RegistrationResult nano_res;
  double nano_icp = measure_ms([&]() {
    nano_res = npcl::registration::icp(nano_source, nano_target, nano_tree,
  ...
  std::cout << "\n  nanoPCL:\n";
  std::cout << "    KdTree build:  " << std::setw(10) << nano_build << " ms\n";
