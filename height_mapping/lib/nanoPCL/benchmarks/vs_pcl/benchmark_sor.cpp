...
int main() {
  ...
  for (size_t NUM_POINTS : point_counts) {
    ...
    // =========================================================================
    {
      ...
      std::cout << "│ ";
      printResult("[PCL] StatisticalOutlierRemoval", pcl_time, NUM_POINTS,
                  pcl_out);
    }
    ...
    // =========================================================================
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        for (size_t i = 0; i < NUM_POINTS; ++i) {
          cloud.add(npcl::Point(pos(gen), pos(gen), pos(gen)),
                    npcl::Intensity(intensity(gen)));
        }

        Timer t;
        auto filtered = npcl::filters::statisticalOutlierRemoval(cloud, K, STD_MUL);
        total += t.elapsed_ms();
      ...
      std::cout << "│ ";
      printResult("[nanoPCL] SOR (copy)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
    ...
    // =========================================================================
    {
      ...
      for (int iter = 0; iter < ITERATIONS; ++iter) {
        ...
        Timer t;
        auto filtered =
            npcl::filters::statisticalOutlierRemoval(std::move(cloud), K, STD_MUL);
        total += t.elapsed_ms();
        nano_out = filtered.size();
        doNotOptimize(nano_out);
      }
      double nano_time = total / ITERATIONS;
      std::cout << "│ ";
      printResult("[nanoPCL] SOR (move)", nano_time, NUM_POINTS, nano_out,
                  pcl_time);
    }
