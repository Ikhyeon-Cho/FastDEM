...
npcl::PointCloud createNanoPclCloud(size_t num_points, bool with_intensity,
                                        bool with_color, bool with_normal) {
  ...
  if (with_color) cloud.enableColor();
  if (with_normal) cloud.enableNormal();
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (with_color) {
      cloud.color().back() =
          npcl::Color{color_dist(gen), color_dist(gen), color_dist(gen)};
    }
    if (with_normal) {
      float nx = normal_dist(gen), ny = normal_dist(gen), nz = normal_dist(gen);
      float len = std::sqrt(nx * nx + ny * ny + nz * nz);
...
int main() {
  ...
  for (size_t num_points : point_counts) {
    ...
    auto r1 = runBenchmark(
        [&]() {
          result_cloud = npcl::from(*pcl_cloud);
    ...
    auto r2 = runBenchmark(
        [&]() {
          result_pcl = npcl::to<pcl::PointCloud<pcl::PointXYZ>>(nano_cloud);
  ...
  for (size_t num_points : point_counts) {
    ...
    auto r2 = runBenchmark(
        [&]() {
          result_pcl = npcl::to<pcl::PointCloud<pcl::PointXYZI>>(nano_cloud);
          doNotOptimize(result_pcl.points[0].x);
        },
        NUM_ITERATIONS, num_points);
  ...
  for (size_t num_points : point_counts) {
    ...
    auto r2 = runBenchmark(
        [&]() {
          result_pcl =
              npcl::to<pcl::PointCloud<pcl::PointXYZRGB>>(nano_cloud);
          doNotOptimize(result_pcl.points[0].x);
        },
        NUM_ITERATIONS, num_points);
  ...
  for (size_t num_points : point_counts) {
    ...
    auto r2 = runBenchmark(
        [&]() {
          result_pcl =
              npcl::to<pcl::PointCloud<pcl::PointXYZINormal>>(nano_cloud);
          doNotOptimize(result_pcl.points[0].x);
        },
        NUM_ITERATIONS, num_points);
  ...
  auto r1 = runBenchmark(
      [&]() {
        result = npcl::from<pcl::PointXYZI>(pcl_ptr);
        doNotOptimize(result[0].x());
      },
      NUM_ITERATIONS, ptr_test_points);
  ...
  auto r2 = runBenchmark(
      [&]() {
        result_ptr = npcl::toPclPtr<pcl::PointXYZI>(nano_cloud);
  ...
  size_t nano_size = bandwidth_points * (sizeof(npcl::Point) + sizeof(float));
  ...
  {
    Timer t;
    auto cloud = npcl::to<pcl::PointCloud<pcl::PointXYZI>>(nano_xyzi);
