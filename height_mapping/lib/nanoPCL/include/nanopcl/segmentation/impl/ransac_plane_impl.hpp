...
class XorShift64 {
 public:
  explicit XorShift64(uint64_t seed = 42) : state_(seed ? seed : 1) {}
...
inline size_t countInliers(const PointCloud& cloud,
                           ...
                           const PlaneModel& model, float threshold) {
  const float nx = model.normal.x();
  const float ny = model.normal.y();
  const float nz = model.normal.z();
  const float d = model.d;
...
inline PlaneModel PlaneModel::fromPoints(const Point& p1, const Point& p2,
                                         const Point& p3) {
  ...
  if (norm < 1e-10f) {
    ...
    return {Eigen::Vector3f::Zero(), std::numeric_limits<float>::quiet_NaN()};
  }
...
inline RansacResult segmentPlane(const PointCloud& cloud,
  ...
  for (int iter = 0; iter < config.max_iterations && iter < adaptive_iterations;
    ...
    while (i2 == i1) i2 = rng.randIndex(n);
    while (i3 == i1 || i3 == i2) i3 = rng.randIndex(n);

    const Point& p1 = cloud[indices[i1]];
    const Point& p2 = cloud[indices[i2]];
    const Point& p3 = cloud[indices[i3]];
  ...
  result.inliers = detail::collectInliers(cloud, indices, best_model,
  ...
  if (config.refine_model && result.inliers.size() >= 3) {
    ...
    if (refined.isValid()) {
      ...
      result.inliers = detail::collectInliers(cloud, indices, refined,
                                              config.distance_threshold);
    }
  }
...
inline std::vector<RansacResult> segmentMultiplePlanes(
    const PointCloud& cloud, const RansacConfig& config, size_t max_planes,
    double min_fitness) {
