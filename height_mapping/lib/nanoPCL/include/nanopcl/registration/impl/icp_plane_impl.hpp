...
#ifndef NANOPCL_REGISTRATION_IMPL_ICP_PLANE_IMPL_HPP
#define NANOPCL_REGISTRATION_IMPL_ICP_PLANE_IMPL_HPP
...
#include "nanopcl/registration/impl/point_to_plane_optimizer.hpp"

namespace npcl {
namespace registration {

template <typename SearchMethod>
RegistrationResult icpPlane(const PointCloud& source, const PointCloud& target,
                            const SearchMethod& target_search,
                            const Eigen::Isometry3d& initial_guess,
                            const ICPPlaneConfig& config) {
  // ==========================================================================
  // Precondition: target must have normals
  // ==========================================================================
  if (!target.hasNormal()) {
    throw std::runtime_error(
        "icpPlane: Target cloud missing normals. "
...
#pragma omp parallel for reduction(+ : sum_dist_sq)
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      // Skip if target normal is invalid (NaN check)
      const auto& normal = target.normal()[nearest->index];
    ...
    // ========================================================================
    // Step 3: Gauss-Newton optimization (Point-to-Plane)
    // ========================================================================
    Eigen::Isometry3d delta_T = detail::solvePointToPlane(
        source, target, T_current, correspondences, config.robust_kernel,
...
RegistrationResult icpPlane(const PointCloud& source, const PointCloud& target,
  ...
  return icpPlane(source, target, searcher, initial_guess, config);
}

}  // namespace registration
}  // namespace npcl

#endif  // NANOPCL_REGISTRATION_IMPL_ICP_PLANE_IMPL_HPP
