...
#include "height_mapping/core/algorithms/voxel_filter.h"
...
VoxelFilter::Stats VoxelFilter::filterInPlace(
...
height_mapping::PointCloud VoxelFilter::filter(
    const height_mapping::PointCloud &cloud, Stats &stats) const {
  height_mapping::PointCloud result = cloud;  // Copy
  stats = filterInPlace(result);              // Modify copy
  return result;
}

VoxelFilter::VoxelKey VoxelFilter::getVoxelKey(
    const height_mapping::Point3D &point) const {
...
height_mapping::Point3D VoxelFilter::computeRepresentativePoint(
    const VoxelData &voxel_data, const VoxelKey &key) const {
