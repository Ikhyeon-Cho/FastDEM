 ...
 */
std::vector<RayTarget> extractTargets(const HeightMap& map,
                                      const PointCloud& cloud) {
  const int cols = map.getSize()(1);
  std::unordered_map<int, RayTarget> cell_map;
  cell_map.reserve(cloud.size());

  for (const auto& point : cloud.xyz()) {
    grid_map::Index idx;
    if (!map.getIndex({point.x(), point.y()}, idx)) continue;
 ...
 */
void traceRayForMaxHeight(const HeightMap& map, float resolution,
 ...
 */
void computeUpperBounds(HeightMap& map, const Eigen::Vector3f& sensor_origin,
...
void applyRaycasting(HeightMap& map, const PointCloud& cloud_in_map,
  ...
  auto downward_cloud = npcl::filters::cropZaxis(
