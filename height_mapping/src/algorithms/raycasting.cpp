...
void traceRaySupercover(HeightMap& map, const grid_map::Index& start,
  ...
  for (int i = 0; i < total_cells - 1; ++i) {
    ...
    if (std::isnan(hit_mask[linear_idx])) {
      ...
      if (!std::isnan(map_z) &&
          map_z > current_z + config.correction_threshold) {
...
void applyRaycasting(HeightMap& map, const PointCloud& scan,
                     ...
                     const RaycastingConfig& config) {
  if (!config.enable_correction || scan.empty()) {
    return;
  }
  ...
  const float min_dist_sq = config.min_ray_distance * config.min_ray_distance;
