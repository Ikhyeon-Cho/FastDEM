...
#ifndef HEIGHT_MAPPING_PPL_STAGES_MULTI_SENSOR_SYNC_H
#define HEIGHT_MAPPING_PPL_STAGES_MULTI_SENSOR_SYNC_H
...
class MultiSensorSync : public ::ppl::Stage<MappingFrame> {
 ...
 public:
  ...
  void configure(const YAML::Node& config) override {
    if (config["buffer_size"]) buffer_size_ = config["buffer_size"].as<size_t>();
    ...
    if (config["merge_policy"]) {
      ...
      if (merge_policy_ != "latest" && merge_policy_ != "exact_sync" &&
        ...
        spdlog::error("[MultiSensorSync] Unknown merge policy: {}, using 'latest'",
                      merge_policy_);
        merge_policy_ = "latest";
      }
    }

    if (config["max_sensors"]) max_sensors_ = config["max_sensors"].as<size_t>();

    spdlog::debug("[MultiSensorSync] buffer_size={}, sync_tolerance={}ms, policy={}",
  ...
  bool process(const std::shared_ptr<MappingFrame>& frame) override {
    ...
    spdlog::debug(
        "[MultiSensorSync] {} sensors, {} processed, {} synchronized, {} merged",
...
#endif  // HEIGHT_MAPPING_PPL_STAGES_MULTI_SENSOR_SYNC_H
