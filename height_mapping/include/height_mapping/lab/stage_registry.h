 ...
 *
 * - PPL_REGISTER_STAGE uses static variable initialization for
 * auto-registration
 * - __attribute__((used)) only prevents compiler optimization, not linker
 * stripping
 * - Static library linking discards unreferenced .o files entirely
 *
 * Must be called once before Pipeline::load(). Typically called in Mapper
 * constructor.
 */
