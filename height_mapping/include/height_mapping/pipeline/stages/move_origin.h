 ...
 *
 * This stage requires TransformLookup service to get robot position.
 */
class MoveOrigin : public flowpipe::Stage {
 public:
  ...
  void configure(const std::map<std::string, std::string> &params) override {
    ...
    if (loadParam(params, "update_mode", mode)) {
      if (mode == "continuous") {
        config.update_mode = algorithms::MoveOrigin::UpdateMode::CONTINUOUS;
      } else if (mode == "threshold") {
        config.update_mode = algorithms::MoveOrigin::UpdateMode::THRESHOLD;
    ...
    LOG_DEBUG(
        getName(), "Configured with update_mode=",
        params.count("update_mode") ? params.at("update_mode") : "continuous");
  }
 ...
 protected:
  void processImpl(flowpipe::Context &ctx) override {
    ...
    auto transform_provider = ctx.getService<TransformLookup>();
