/*
 * elevation_quantile.h
 *
 * Elevation layer updater using P² quantile estimation algorithm.
...
#ifndef HEIGHT_MAPPING_CORE_UPDATER_ELEVATION_QUANTILE_H
#define HEIGHT_MAPPING_CORE_UPDATER_ELEVATION_QUANTILE_H
 ...
 * Usage:
 * @code
 *   updater::QuantileElevation p2_elev(config);
...
class QuantileElevation {
 public:
   ...
   */
  QuantileElevation()
      : QuantileElevation(0.01f, 0.16f, 0.50f, 0.84f, 0.99f, 3, 0.0f) {}
   ...
   * @param max_sample_count Maximum sample count for fading memory (0 = disabled)
   */
  QuantileElevation(float dn0, float dn1, float dn2, float dn3, float dn4,
  ...
  void finalize() {
    ...
    *elevation_mat_ = *q_mat_[elev_idx];  // elevation = q[elev_idx]
    ...
    // Confidence bounds: directly from quantiles (no normal distribution assumption)
...
#endif  // HEIGHT_MAPPING_CORE_UPDATER_ELEVATION_QUANTILE_H
