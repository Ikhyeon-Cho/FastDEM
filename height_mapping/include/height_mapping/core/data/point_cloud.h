...
class PointCloud {
 public:
  ...
  class PointView {
   public:
    ...
    void setIntensity(float val) {
      if (cloud_->hasIntensity()) (*cloud_->intensity_)[index_] = val;
    }
    ...
    void setColor(const std::array<uint8_t, 3> &val) {
      if (cloud_->hasColor()) (*cloud_->color_)[index_] = val;
    }
  ...
  void resize(size_t n) {
    points_.resize(n);
    if (intensity_) intensity_->resize(n);
    if (color_) color_->resize(n);
  }
  void clear() {
    points_.clear();
  ...
  void push_back(const Point3D &point) {
    ...
    if (intensity_) intensity_->push_back(0.0f);  // Default intensity
    if (color_) color_->push_back({0, 0, 0});     // Default color
  }
  void push_back(Point3D &&point) {
    points_.push_back(std::move(point));
    if (intensity_) intensity_->push_back(0.0f);
    if (color_) color_->push_back({0, 0, 0});
  }
  ...
  void addPoint(const Point3D &point, float intensity_val = 0.0f,
    ...
    if (color_) color_->push_back(color_val);
  }
  ...
  void filterInPlace(Predicate pred) {
    ...
    for (size_t read_idx = 0; read_idx < points_.size(); ++read_idx) {
      if (pred(ConstPointView(this, read_idx))) {
        ...
        if (write_idx != read_idx) {
          ...
          if (hasColor()) (*color_)[write_idx] = (*color_)[read_idx];
        }
        write_idx++;
      }
    }
    ...
    if (hasIntensity()) intensity_->resize(write_idx);
    if (hasColor()) color_->resize(write_idx);
  }
