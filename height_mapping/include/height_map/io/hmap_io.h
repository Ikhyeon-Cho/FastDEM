...
#ifndef HEIGHT_MAP_IO_HMAP_IO_H
#define HEIGHT_MAP_IO_HMAP_IO_H
...
inline bool loadHMap(const std::string& filename, HeightMap& map) {
  ...
  if (std::strncmp(header.magic, "HMAP", 4) != 0) {
    std::cerr << "[hmap_io] Error: Invalid file format (bad magic)" << std::endl;
    return false;
  }
