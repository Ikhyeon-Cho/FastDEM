...
/**
 * @brief Convert ElevationMap to ROS2 GridMap message
...
inline void toMessage(const fastdem::ElevationMap& map,
  ...
  // Header (top-level in ROS2, unlike ROS1's info.header)
  msg.header.stamp = rclcpp::Time(static_cast<int64_t>(map.getTimestamp()));
...
inline sensor_msgs::msg::PointCloud2 toPointCloud2(
  ...
  return nanopcl::to(cloud, map.getFrameId(),
                     rclcpp::Time(static_cast<int64_t>(map.getTimestamp())));
}

/**
 * @brief Create map boundary marker for RViz visualization
 */
inline visualization_msgs::msg::Marker toRegionMarker(
    const fastdem::ElevationMap& map) {
