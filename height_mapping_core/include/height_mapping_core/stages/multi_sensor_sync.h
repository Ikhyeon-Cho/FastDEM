 ...
 */
class MultiSensorSyncStage : public pipeline::Stage {
public:
  MultiSensorSyncStage();
   ...
   *        - "sync_tolerance_ms": Time tolerance for synchronization in milliseconds
...
private:
  ...
  struct CloudBuffer {
    std::deque<std::shared_ptr<PointCloudXYZ>> clouds;
