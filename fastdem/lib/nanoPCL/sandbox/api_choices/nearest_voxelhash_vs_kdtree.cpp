...
PointCloud generateNextScan(const LidarSpec& spec, float max_range, float dx, float dy, float dyaw, unsigned seed) {
...
// =============================================================================
void benchmarkFrameToFrame(const LidarSpec& spec, float max_range, float search_radius, int rounds) {
  ...
  for (int r = 0; r < rounds; ++r) {
    ...
    flushCache();
    {
      ...
      for (size_t i = 0; i < source.size(); ++i) {
        if (searcher.nearest(source[i], search_radius))
          found++;
      }
...
// =============================================================================
void benchmarkScanToMap(const LidarSpec& spec, size_t map_scans, float max_range, float search_radius, int rounds) {
  ...
  Timer tb1;
  voxel_searcher.build(map);
  double vb = tb1.elapsed_ms();
  Timer tb2;
  kdtree_searcher.build(map);
  double kb = tb2.elapsed_ms();
  ...
  for (int r = 0; r < rounds; ++r) {
    ...
    flushCache();
    {
      ...
      for (size_t i = 0; i < scan.size(); ++i) {
        if (voxel_searcher.nearest(scan[i], search_radius))
          found++;
      }
      voxel_times.push_back(t.elapsed_ms());
      doNotOptimize(found);
    ...
    flushCache();
    {
      ...
      size_t found = 0;
      for (size_t i = 0; i < scan.size(); ++i) {
        if (kdtree_searcher.nearest(scan[i], search_radius))
          found++;
      }
      kdtree_times.push_back(t.elapsed_ms());
      doNotOptimize(found);
  ...
  for (float radius : radii) {
    ...
    for (int r = 0; r < rounds; ++r) {
      flushCache();
      {
        ...
        size_t f = 0;
        for (size_t i = 0; i < cloud.size(); ++i)
          if (vs.nearest(cloud[i], radius))
            f++;
        vt.push_back(t.elapsed_ms());
        doNotOptimize(f);
      }
      ...
      flushCache();
      {
        ...
        size_t f = 0;
        for (size_t i = 0; i < cloud.size(); ++i)
          if (ks.nearest(cloud[i], radius))
            f++;
        kt.push_back(t.elapsed_ms());
        doNotOptimize(f);
      }
    }
