...
PointCloud generateCloud(size_t num_points, int num_channels, unsigned seed = 42) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (num_channels >= 1)
      cloud.intensity().back() = val_dist(gen);
    if (num_channels >= 2)
      cloud.time().back() = val_dist(gen);
    if (num_channels >= 3)
...
PointCloud baselineCopy(const PointCloud& cloud, Predicate pred) {
  ...
  for (size_t i = 0; i < n; ++i) {
    if (pred(pts[i])) {
      ...
      if (color)
        result.color().push_back(color[i]);
      if (label)
        result.label().push_back(label[i]);
    }
  }
...
void printResults(const std::vector<BenchResult>& results, size_t num_points, const std::string& baseline_name = "") {
...
void runScenario(const std::string& title, size_t num_points, int num_channels, float pass_rate, int iterations) {
  ...
  auto pred_ref = [=](auto p) {
    float dist_sq = (p.xyz() - origin).squaredNorm();
    return dist_sq >= min_dist_sq && dist_sq <= max_dist_sq;
  };
