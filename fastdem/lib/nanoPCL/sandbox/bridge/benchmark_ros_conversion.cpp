...
sensor_msgs::PointCloud2 createRosMessage(size_t num_points, bool with_intensity, bool with_ring, bool with_time) {
  ...
  if (with_ring)
    addField("ring", sensor_msgs::PointField::UINT16);
  if (with_time)
    addField("t", sensor_msgs::PointField::FLOAT32);
...
nanopcl::PointCloud createNanoPclCloud(size_t num_points, bool with_intensity, bool with_ring, bool with_time) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (with_ring)
      cloud.ring().back() = ring_dist(gen);
    if (with_time)
      cloud.time().back() = time_dist(gen);
  }
...
void printResult(const std::string& name, const BenchmarkResult& r, size_t num_points) {
...
int main(int argc, char** argv) {
  ...
  for (size_t num_points : point_counts) {
    ...
    auto r1 = runBenchmark(
        [&]() {
          result_cloud = nanopcl::from(ros_msg);
    ...
    auto r2 = runBenchmark(
        [&]() {
          result_msg = nanopcl::to<sensor_msgs::PointCloud2>(nano_cloud);
    ...
    auto r3 = runBenchmark(
        [&]() {
          auto cloud = nanopcl::from(ros_msg);
          auto msg = nanopcl::to<sensor_msgs::PointCloud2>(cloud);
  ...
  // ============================================================================
  // 4. COMPARISON WITH PROCESSING COST
  // ============================================================================
  printHeader("4. CONVERSION vs PROCESSING COST COMPARISON");
  ...
  auto voxel_result = runBenchmark(
      [&]() {
        auto filtered =
            nanopcl::filters::voxelGrid(nano_cloud, 0.1f);
  ...
  auto transform_result = runBenchmark(
      [&]() {
        auto transformed = nanopcl::transformCloud(nano_cloud, tf);
  ...
  auto pipeline_result = runBenchmark(
      [&]() {
        auto cloud = nanopcl::from(ros_msg);
        cloud = nanopcl::filters::voxelGrid(std::move(cloud), 0.1f);
        cloud = nanopcl::transformCloud(std::move(cloud), tf);
        auto out_msg = nanopcl::to<sensor_msgs::PointCloud2>(cloud);
        doNotOptimize(out_msg.data[0]);
      },
      NUM_ITERATIONS,
      comparison_points);
  printResult("Full pipeline (conv+filter+tf+conv)", pipeline_result, comparison_points);
