...
int main(int argc, char** argv) {
  ...
  auto stripToXYZI = [](const sensor_msgs::PointCloud2& in) {
    ...
    fi.datatype = sensor_msgs::PointField::FLOAT32;
    fi.count = 1;
  ...
  for (size_t i = 0; i < std::min(size_t(10), test_messages.size()); ++i) {
    ...
    auto c2 = nanopcl::from(*test_messages[i]);
  }
  ...
  std::vector<nanopcl::PointCloud> nanopcl_clouds;
  ...
  for (const auto& msg : test_messages) {
    ...
    nanopcl_clouds.push_back(nanopcl::from(*msg));
  }
