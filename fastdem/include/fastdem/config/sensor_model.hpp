// SPDX-License-Identifier: BSD-3-Clause
// Copyright (c) 2024 Ikhyeon Cho <tre0430@korea.ac.kr>

/*
 * sensor_model.hpp
 *
 * Sensor model configuration for measurement uncertainty.
 *
 *  Created on: Dec 2024
 *      Author: Ikhyeon Cho
 *   Institute: Korea Univ. ISR (Intelligent Systems & Robotics) Lab
 *       Email: tre0430@korea.ac.kr
 */

#ifndef FASTDEM_CONFIG_SENSOR_MODEL_HPP
#define FASTDEM_CONFIG_SENSOR_MODEL_HPP

namespace fastdem {

/// Sensor model type for measurement uncertainty computation.
enum class SensorType {
  Constant,  ///< Isotropic constant uncertainty
  LiDAR,     ///< Range-dependent LiDAR model
  RGBD       ///< Depth-dependent RGB-D model (Nguyen et al. 2012)
};

namespace config {

/**
 * @brief Sensor model configuration.
 */
struct Sensor {
  SensorType type = SensorType::LiDAR;

  /// @name LiDAR model parameters
  /// @{
  float range_noise = 0.02f;    ///< Range uncertainty σ_r [m]
  float angular_noise = 0.001f; ///< Angular uncertainty σ_θ [rad]
  /// @}

  /// @name RGB-D model parameters (Nguyen et al. 2012)
  /// @{
  float normal_a = 0.001f;       ///< Base depth noise [m]
  float normal_b = 0.002f;       ///< Quadratic coefficient [m⁻¹]
  float normal_c = 0.4f;         ///< Optimal depth [m]
  float lateral_factor = 0.001f; ///< Lateral noise factor
  /// @}

  /// Constant model uncertainty σ [m]
  float constant_uncertainty = 0.1f;
};

}  // namespace config
}  // namespace fastdem

#endif  // FASTDEM_CONFIG_SENSOR_MODEL_HPP
